...
Duas formas de observar a distribuição destas médias de áreas por estabelecimento podem ser vista abaixo:
...
quantile(df\$modulos_fiscais_familiar, na.rm = T)

mun \<- read_municipality() \|\> rename(municipio = name_muni, uf = abbrev_state) \|\> mutate(municipio = tolower(municipio)) \|\> left_join(df) \|\> na.omit() \|\> mutate(across(modulos_fiscais_familiar, .fn = function(x) cut(x, quantile(x, 0:10/10)))) \|\> mutate(across(modulos_fiscais_agro_negocio, .fn = function(x) cut(x, quantile(x, 0:10/10))))

mun \|\> ggplot() + geom_sf(aes(fill = modulos_fiscais_agro_negocio), color=NA) + scale_fill_manual(values = pal) + theme_void()
